...
rtsp_init(const char *pname, const char *rtsp_uri) {
    ...
    data->source  = gst_element_factory_make ( "uridecodebin", "decodebin0");
    ...
    if (!DISPLAY) {
        if ( !data->pipeline || !data->source || !data->tee || !data->queue_appsink || !data->appsink) {
            g_printerr("One element could not be created.\n");
        }
    }else{
        if ( !data->pipeline || !data->source || !data->tee || !data->queue_appsink || !data->queue_displaysink  || !data->displaysink || !data->appsink) {
            g_printerr("One element could not be created.\n");
        }
        // Configure rksink
...
rtsp_destroy (struct CustomData *data)
{
    ...
    gst_bin_remove_many (GST_BIN(data->pipeline), data->source, data->tee, NULL);
