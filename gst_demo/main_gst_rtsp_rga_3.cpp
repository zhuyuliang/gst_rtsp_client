...
// pad_probe_source (GstPad * pad, GstPadProbeInfo * info, gpointer user_data)
// {
...
//   if (videoinfo != NULL){
//     g_print("video %d",videoinfo->height);
//   }
  
//   // switch (GST_VIDEO_INFO_FORMAT (&(videoinfo))) {
...
buffer_to_file (struct CustomData *dec, GstBuffer * buf,  char *srcBuffer_264,char *srcBuffer_265,  char *dstBuffer)
{
  ...
  im_rect src_rect = {0, 0, VIDEO_WIDTH, VIDEO_HEIGHT};
  //g_print("imcrop %d",src_rect.width);
  imcrop(dst,dst_output,src_rect);

  // cv::Mat img(VIDEO_HEIGHT , VIDEO_WIDTH , CV_8UC3, dstBuffer);  
