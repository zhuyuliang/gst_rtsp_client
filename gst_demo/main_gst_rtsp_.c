...
{
  struct CustomData *dec = user_data;
...
on_src_tee_added(GstElement *element, GstPad *pad, gpointer data) {
    GstPad *sinkpad;
    struct CustomData *decoder = data;
...
rtsp_init(const char *pname, const char *rtsp_uri) {
    ...
    */
    ...
    //videoconvert caps
    //current_src_caps = gst_app_sink_get_caps( (GstAppSink *) data->appsink);
    ...
    //     return 0;
    // }
    // if (gst_element_link_pads_filtered (data->videoconvert, "src", pano.panorama, panopadnames[i], vcaps)==FALSE){
    //     g_print("44cant link %s\n", panopadnames[i]);
    // }
    //gst_element_link_filtered(data->videoconvert, data->appsink, new_src_caps);
    ...
    // if (gst_element_link_filtered(data->videoconvert, data->appsink, new_src_caps)==FALSE){
		...
		// }
    // //gst_caps_unref((GstCaps *) current_src_caps);
    // //gst_app_sink_set_caps( (GstAppSink * ) data->appsink, new_src_caps);
