...
//rtspsrc on-sdp
static void
rtspsrc_on_sdp (GstElement * rtspsrc, GstSDPMessage * sdp, gpointer data) {
   g_print("rtspsrc_on_sdp \n");
   g_print( "dp %s \n",sdp->uri);
}

//rtspsrc pad-removed
static void
rtspsrc_pad_removed (GstElement *element, GstPad *pad, gpointer data) {
    g_print("rtspsrc_pad_removed \n");
}

static void
my_handle_request (GstElement *element, gpointer *arg1, gpointer *arg2, gpointer data) {
   g_print("my_handle-request \n");
}

static gboolean
my_before_send (GstElement *element, GstRTSPMessage *message, gpointer data) {
    g_print("my_before_send \n");
    return TRUE;
}
...
//rtspsrc
static GstPadProbeReturn
rtspsrc_pad_probe (GstPad * pad, GstPadProbeInfo * info, gpointer user_data)
{
  // struct CustomData *dec = (struct CustomData *) user_data;
  // GstEvent *event = GST_PAD_PROBE_INFO_EVENT (info);
  // GstCaps *caps;
  ...
  g_print("rtspsrc_pad_probe block \n");
...
on_src_decodebin_added(GstElement *element, GstPad *pad, gpointer data) {
    ...
    // printf("%s\n", p_caps, ", ", description,"\n");
    // g_free(description);
    // gst_pad_add_probe (pad, GST_PAD_PROBE_TYPE_BUFFER, rtspsrc_pad_probe, data, NULL);
...
// static void *
// reconnect_gst (gpointer user_data) {

//   struct CustomData *data = (struct CustomData *) user_data;

//   //gst_element_release_request_pad (data->rtspsrc, gst_element_get_static_pad(data->rtspsrc, "src"));
//   gst_element_unlink ( data->rtspsrc ,data->decode);
//   gst_bin_remove_many( GST_BIN(data->pipeline), data->rtspsrc, NULL);
//   // gst_object_unref ( data->rtspsrc);
//   data->rtspsrc = gst_element_factory_make ( "rtspsrc", "rtspsrc1");
...
//   gst_bin_add_many(GST_BIN(data->pipeline), data->rtspsrc, NULL);

//   g_signal_connect(data->rtspsrc, "pad-added", G_CALLBACK( on_src_decodebin_added), data);
//   g_signal_connect( data->rtspsrc, "pad-removed", G_CALLBACK( rtspsrc_pad_removed), data);
//   g_signal_connect (data->rtspsrc, "on-sdp", G_CALLBACK( rtspsrc_on_sdp), data);

//   //gst_element_unlink ( data->decode ,data->tee);
//   gst_bin_remove_many( GST_BIN(data->pipeline), data->decode, NULL);
//   // gst_object_unref ( data->rtspsrc);
//   data->decode = gst_element_factory_make ( "decodebin", "decodebin1");
//   gst_bin_add_many(GST_BIN(data->pipeline), data->decode, NULL);
...
rtsp_init(const char *pname, const char *rtsp_uri) {
    ...
    */
    ...
    // g_signal_connect ( data->rtspsrc, "pad-removed", G_CALLBACK( rtspsrc_pad_removed), data);
    // g_signal_connect ( data->rtspsrc, "on-sdp", G_CALLBACK( rtspsrc_on_sdp), data);
    // g_signal_connect ( data->rtspsrc, "handle-request", G_CALLBACK( my_handle_request), data);
    // g_signal_connect ( data->rtspsrc, "before-send", G_CALLBACK( my_before_send), data);
